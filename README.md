...
一个现代化的 Web 界面，通过 rosbridge 连接 ROS 系统，提供左侧主视图区域与右侧快捷操作栏的布局，可在 3D 导航视图、摄像头视图与地图编辑器之间快速切换，并通过预设按钮发布常用控制指令。
...
- **多种编辑工具**（每个工具有不同的光标提示）：
  - **指针模式**：左键拖动地图查看（光标：手形）
  - **画笔工具**：自由绘制，支持自由区域/障碍/未知三种笔刷（光标：十字）
  - **框选工具**：拖拽框选矩形区域并批量填充（光标：十字）
  - **橡皮擦**：清除区域变为自由空间（光标：格子）
- 可调节画笔大小（1-32 像素）
- 一键操作：清空地图、反转颜色、导出为 PGM 文件
- 保存至服务器或导出到本地下载
...
4. **摄像头视图**
   - 默认订阅 `/camera/image_raw/compressed` 话题 (`sensor_msgs/CompressedImage`)
   - 自动解码并显示图像数据
...
- 图像话题：`/camera/image_raw/compressed`
